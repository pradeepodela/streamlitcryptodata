...
def download_button(object_to_download, download_filename, button_text, pickle_it=False):
    ...
    if pickle_it:
        ...
        if isinstance(object_to_download, bytes):
            ...
            print(object_to_download)
            #sdf = pd.read_csv(object_to_download)
            #st.write(sdf)
            towrite.seek(0)
...
if __name__ == '__main__':
    ...
    if submit_btn:
        ...
        st.write(coindcx_df)
        print(coindcx_df.shape[0])
        # dft = pd.to_datetime(coindcx_df['dat'])
        
        st.title("**Plot the data Data**")
        
        fig, ax = plt.subplots()
        ax.plot(coindcx_df['date_time'], coindcx_df['high'])
        st.pyplot(fig)
